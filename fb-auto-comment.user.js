...
(function() {
    ...
    // Detect Facebook temporary blocks / checkpoints and stop to protect the account
    function isFbBlocked() {
        if (!document.body) return false;
        const txt = document.body.innerText.toLowerCase();
        return /bị chặn|chặn tạm thờ|hành động quá nhanh|quá nhanh|temporary block|you're temporarily blocked|checkpoint|xác minh|verify your account|hạn chế/i.test(txt);
    }
    ...
    function getPostIdFromHref(href = '') {
        const m = href.match(/(?:\/posts\/|\/permalink\/|\/videos\/|\/reel\/|\/share\/|\/watch\/|fbid=|story_fbid=)(\d+)/);
        return m ? m[1] : null;
    }
        ...
        if (postsMap.size === 0) {
            const anchors = [...document.querySelectorAll('a[href]')]
                .filter(a => {
                    ...
                    return /\/posts\/\d+|\/permalink\/\d+|\/videos\/\d+|\/reel\/\d+|\/share\/\d+|\/watch\/\d+|[?&]fbid=\d+|[?&]story_fbid=\d+/.test(href);
                });
    ...
    function findCommentActivator(post) {
        ...
        const nodes = [...post.querySelectorAll('div, span, [role="button"]')].filter(el => {
            ...
            const hasMainSignal =
                ...
                txt.includes('bình luận') ||
                txt.includes('nhận xét') ||
                txt.includes('write a comment') ||
                txt.includes('write an answer') ||
                txt.includes('write a public comment') ||
    ...
    async function processPostWorkflow(post) {
        ...
        if (isFbBlocked()) {
            isRunning = false;
            log('🛑 Phát hiện Facebook chặn/tạm khóa. Dừng bot.', 'error');
            return;
        }
        ...
        try {
            ...
            if (confirmedTier2) {
                donePostIds.add(postId);
                commentCount++;
                const statCommentsEl = document.getElementById('stat-comments-sent');
                if (statCommentsEl) statCommentsEl.innerText = commentCount;
            ...
            const retries = (postRetryCounts.get(postId) || 0) + 1;
            postRetryCounts.set(postId, retries);
            if (retries >= 3) {
                log(`⛔ Bỏ qua bài ${postId} sau ${retries} lần thử.`, 'error');
                donePostIds.add(postId);
            }
            const rect = post.getBoundingClientRect();
